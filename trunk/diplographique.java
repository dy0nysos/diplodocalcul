class diplographique extends Program{
    //variables globale pour la partie graphique
    Image background=newImage("background.jpeg");
    int cptavcmt=0; //compteur permettant de savoir ou l'on en est graphiquement
    String name,surname;

    //variables globale pour l'algorithme
    ...
    void algorithm(){
	//variables pour la partie graphique
	Image logo=newImage("diplodocus.jpeg");

	//affichage de l'ecran d'accueil
	//mis en place du logo
	drawImage(background,copyAndResize(logo,"petit-logo",70,50),5,5);
	//misen place du texte
	setColor(background,RGBColor.BLACK);
	setNewFont(background, "COLIBRI","ITALIC", 32);
	drawString(background, "Diplodocalcul", 100, 50);
	drawString(background, "Bonjour et bienvenue",120,250);
	drawString(background, "sur Diplodocalcul",120,300);
	//mis en place de la première question
	setText(background,"Tape ton prénom ici");
	setMessage(background,"Prénom?");
	//nommage du bouton
	addButton(background,"OK");
	//affichage
	show(background);

	//variables pour la partie algorithmique
